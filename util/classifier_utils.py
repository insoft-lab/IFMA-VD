...
def classifier_output(classifier_name, data_train, label_train, data_test, label_test, grid_sear=False):
    ...
    if (classifier_name == "LogisticRegression"):
        ...
        if (grid_sear == True):
            ...
            # precision, recall, fmeasure, true_sum, MCM = metrics.precision_recall_fscore_support(labels,
            #                                                                                             predictions,
            #                                                                                             average='binary',
            #                                                                                             pos_label=1)
            recall = metrics.recall_score(label_test, predprob)
        ...
        if (grid_sear == False):
            ...
            return predprob_auc, predprob, precision, recall, fmeasure, auc, mcc, accuracy
        if (grid_sear == True):
            parameters = {'criterion': ['gini', 'entropy'], 'max_depth': [30, 50, 60, 100],
        ...
        if (grid_sear == False):
            ...
            return predprob_auc, predprob, precision, recall, fmeasure, auc, mcc, accuracy
        if (grid_sear == True):
            parameters = [{'kernel': ['linear'], 'C': [1, 10, 100, 100]},
        ...
        if (grid_sear == True):
            ...
            parameters = {'n_estimators': range(10, 71, 10), 'max_depth': range(3, 14, 2),
                          'min_samples_split': range(10, 201, 20), 'min_samples_leaf': range(10, 60, 10)}
        ...
        if (grid_sear == False):
            ...
            return predprob_auc, predprob, precision, recall, fmeasure, auc, mcc, accuracy
        if (grid_sear == True):
            parameters = {"hidden_layer_sizes": [(100,), (100, 30)], "solver": ['adam', 'sgd'],
                          "max_iter": [200, 400], }
            gsearch = GridSearchCV(rf, parameters, scoring='f1', cv=5, n_jobs=8)
            gsearch.fit(data_train, label_train)
            predprob = gsearch.predict(data_test)
